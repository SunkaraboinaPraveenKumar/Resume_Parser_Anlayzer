...
try:
    connection = pymysql.connect(host='localhost', user='root', password='1234', db='resume_parser')
...
def run():
    ...
    link = '[©Developed by Praveen Kumar](https://www.linkedin.com/in/sunkaraboina-praveen-84594a256/)'
    st.sidebar.markdown(link, unsafe_allow_html=True)

    # Create the DB
    db_sql = """CREATE DATABASE IF NOT EXISTS RESUME_PARSER;"""
    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'praveen' and ad_password == 'prav27':
                st.success("Welcome Praveen Kumar!")
                ...
                # Check columns before accessing them
                # Check columns before accessing them
                print("Available columns in plot_data:", plot_data.columns.tolist())

                print(plot_data.dtypes)

                print("Data for Predicted_Field:", plot_data['Predicted_Field'].head())
                print("Data for User_level:", plot_data['User_level'].head())

                # Decode byte strings in 'Predicted_Field' and 'User_level'
                if 'Predicted_Field' in plot_data.columns:
                    plot_data['Predicted_Field'] = plot_data['Predicted_Field'].apply(
                        lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)
                    value_counts = plot_data['Predicted_Field'].value_counts()
                    labels = value_counts.index.tolist()
                    values = value_counts.values.tolist()

                    st.subheader("**Pie-Chart for Predicted Field Recommendation**")
                    fig = px.pie(names=labels, values=values, title='Predicted Field according to the Skills')
                    st.plotly_chart(fig)  # Directly pass the figure here
                else:
                    st.error("Predicted_Field column not found in the data.")

                # Pie chart for User's Experienced Level
                if 'User_level' in plot_data.columns:
                    plot_data['User_level'] = plot_data['User_level'].apply(
                        lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)
                    value_counts = plot_data['User_level'].value_counts()
                    labels = value_counts.index.tolist()  # Get unique labels
                    values = value_counts.values.tolist()  # Get corresponding counts
                    ...
                    st.error("User_level column not found in the data.")
            else:
